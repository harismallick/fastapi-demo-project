...
async def test_forgot_password_sends_email(client: AsyncClient):
    ...
    # Patch where the method is looked-up, not where the method is defined
    # In the example below, send_password_reset_email function is defined in email_utils.py
    # But for mocking to work correctly, we're calling it from the users.py file, which performs the lookup
    with patch(
